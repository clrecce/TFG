...
export default function Proyectos() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchProyectos();
    } catch (error) { console.error("Error guardando proyecto:", error); }
  };

  // NUEVO: Función para eliminar
  const handleDelete = async (id) => {
    if (!window.confirm("¿Seguro que deseas eliminar este proyecto y todos sus requisitos asociados?")) return;
    try {
  ...
  return (
    <div style={{ padding: '30px', color: 'white', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <p style={{ color: '#a5b4fc', marginBottom: '30px' }}>Iniciativa y registro de nuevos proyectos tecnológicos sostenibles.</p>
      ...
      <div style={{ backgroundColor: '#252536', padding: '20px', borderRadius: '8px', border: '1px solid #3a3a52' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '15px', alignItems: 'flex-end' }}>
          <div style={{ flex: 2 }}>
            ...
            <input type="text" required placeholder="Ej: Sistema Hospitalario Green" value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} style={{ width: '100%', padding: '10px', borderRadius: '5px', backgroundColor: '#1e1e2f', color: 'white', border: '1px solid #3a3a52', boxSizing: 'border-box' }} />
          </div>
          <div style={{ flex: 1 }}>
          ...
          <button type="submit" style={{ padding: '11px 20px', backgroundColor: '#4ade80', color: '#1e1e2f', fontWeight: 'bold', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
            + Crear Proyecto
          </button>
        </form>
      </div>

      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '30px', backgroundColor: '#252536', borderRadius: '8px', overflow: 'hidden' }}>
        <thead>
          <tr style={{ backgroundColor: '#1a1a24', color: '#4ade80', textAlign: 'left' }}>
            <th style={{ padding: '15px' }}>ID</th>
            <th style={{ padding: '15px' }}>Nombre del Proyecto</th>
            <th style={{ padding: '15px' }}>Fecha de Inicio</th>
        ...
        <tbody>
          {proyectos.map((p) => (
            <tr key={p.id} style={{ borderTop: '1px solid #3a3a52' }}>
              <td style={{ padding: '15px', color: '#a5b4fc' }}>PRJ-{p.id.toString().padStart(3, '0')}</td>
              <td style={{ padding: '15px', fontWeight: 'bold' }}>{p.nombre}</td>
              <td style={{ padding: '15px' }}>{p.fecha_inicio}</td>
              <td style={{ padding: '15px' }}>
                <span style={{ backgroundColor: p.estado === 'Desplegado' ? '#064e3b' : '#1e3a8a', color: p.estado === 'Desplegado' ? '#34d399' : '#93c5fd', padding: '5px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>
                  {p.estado}
                </span>
              </td>
              <td style={{ padding: '15px', textAlign: 'center' }}>
                <button onClick={() => handleDelete(p.id)} style={{ background: 'transparent', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: '16px' }} title="Eliminar Proyecto">🗑️</button>
              </td>
            </tr>
          ))}
          {proyectos.length === 0 && (<tr><td colSpan="5" style={{ padding: '20px', textAlign: 'center', color: '#888' }}>No hay proyectos activos.</td></tr>)}
        </tbody>
      </table>
    </div>
  );
